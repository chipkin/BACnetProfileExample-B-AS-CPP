# Tutorial - extending and reviewing the B-AS example
...
**Require a DeviceCommunicationControl password** - set `DCC_PASSWORD` in
`main.cpp` to a non-empty string. With it empty (the shipped default), the
device accepts any `DeviceCommunicationControl` request; with it set, a
mismatched password is rejected with `password-failure`. See the
`DeviceCommunicationControl` callback's comments for the length-first
comparison rule (a wire `CharacterString` may contain embedded NULs, so never
`strcmp` it).

**Once AA-AS-B is unblocked** - see [TODO.md](TODO.md) for the exact stack gap.
Implementing it means seeding an Authorization Policy/Cache the stack can
evaluate against an incoming Access-Authorization request, and adding the
Access User / Access Rights / Access Credential objects Clause 17 defines.
Re-run the grep in `TODO.md` against the pinned stack header before assuming
this is still blocked; if the export has moved to the customer-facing DLL
surface, implement AA-AS-B here and delete the TODO entry rather than leaving
this example claiming a gap that no longer exists.
...
Going beyond this DS/DM base means implementing the Access Authorization
exchange itself (AA-AS-B) — currently blocked, see [TODO.md](TODO.md) — or a
richer profile entirely; see the series table in [README.md](README.md).
...
4. Confirm the services you do **not** implement are still rejected - for B-AS,
   WriteProperty and ReadPropertyMultiple to any object.
5. Exercise `DeviceCommunicationControl`: `disable-initiation` and `enable` get
   a SimpleACK; the deprecated `disable` (1) is rejected by the stack itself
   (`service-request-denied`) at Protocol_Revision >= 20; a wrong password (if
   `DCC_PASSWORD` is set) is rejected with `password-failure`.
...
```bash
python tools/gen-objects-properties.py BACnetProfileExample-B-AS-CPP            # rewrite
python tools/gen-objects-properties.py BACnetProfileExample-B-AS-CPP --check    # fail if stale
```
...
| `DeviceCommunicationControl` with `enableDisable = disable (1)` is rejected | Expected. Protocol_Revision >= 20 deprecates the plain "disable" value; the stack itself rejects it with `service-request-denied` even though this callback would accept it. Send `disable-initiation` (2) instead. |
| `DeviceCommunicationControl` rejected with `password-failure` | `DCC_PASSWORD` is set to a non-empty value and the request's password did not match it byte-for-byte (comparison is length-first, not `strcmp`, because a wire string may contain embedded NULs). |
| A request for Access Authorization (AA-AS-B) fails / is not answered | Expected - not implemented. See [TODO.md](TODO.md) for the exact stack gap (`BACnetStack_AddAuthorizationPolicy` and siblings are test-tool-only at the series pin) and the filed issue [chipkin/cas-bacnet-stack#2043](https://github.com/chipkin/cas-bacnet-stack/issues/2043). |
