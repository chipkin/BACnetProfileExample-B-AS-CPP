...
// =============================================================================
// BACnet Profile Example - B-AS (Authorization Server) - C++
//
// This example shows the CAS BACnet Stack customer-facing surface needed for the
// BACnet "B-AS" (Authorization Server) device profile's DS/DM base:
//
//     DS-RP-B   - respond to ReadProperty requests
//     DM-DDB-B  - respond to Who-Is with I-Am, and announce on start-up
//     DM-DOB-B  - respond to Who-Has with I-Have
//     DM-DCC-B  - respond to DeviceCommunicationControl messages: a management
//                 station can tell this device to stop, or resume, communicating.
//
// AA-AS-B (the Access Authorization service exchange, ASHRAE 135 Clause 17) is
// NOT implemented here. It is not faked: see TODO.md for the specific stack gap
// and its evidence. Short version - at the pinned commit, the Device-Authorization
// seeding exports the profile would need (BACnetStack_AddAuthorizationPolicy and
// friends) are not part of the customer-facing DLL surface this repo is
// restricted to (see the "Customer-facing interface only" global constraint),
// so AA-AS-B cannot be executed here.
//
// The device exposes the same three read-only sensor objects plus the Network
// Port object every base-series example does. Each object is given a colour name
// (a convention shared across this example series):
//
//     Device 389023          "Rainbow"    (instance configurable with --deviceID)
...
// -----------------------------------------------------------------------------
static const char* APP_NAME = "BACnet Authorization Server (B-AS) Example - C++";
static const char* APP_VERSION = "1.0.0";

// The device instance. BACnet requires this to be configurable, so it defaults
// to 389023 and can be overridden on the command line with --deviceID. Keep it
// configurable in your product: it must be unique across the internetwork.
static uint32_t g_deviceInstance = 389023;
...
static const char* DEVICE_DESCRIPTION =
    "Chipkin CAS BACnet Stack example - B-AS (Authorization Server) profile. "
    "Demonstrates DS-RP-B + DM-DDB-B + DM-DOB-B + DM-DCC-B with read-only "
    "sensor inputs. AA-AS-B not implemented - see TODO.md.";
...
static const char* MODEL_NAME = "CAS BACnet Stack Example - B-AS";
...
// requires it to match if configured, or accepts any request if left "". A real
// deployment should set a non-empty password here.
...
bool DeviceCommunicationControl(const uint32_t deviceInstance, const uint8_t enableDisable,
                                ...
                                uint32_t* errorCode) {
    if (deviceInstance != g_deviceInstance) {
        // Not our device. Set *errorCode even here - a false return with
        // *errorCode unset ships "Error Code = success(84)", which is
    ...
    //
    // Compare by LENGTH FIRST, then bytes: a BACnet CharacterString may legitimately
    // contain embedded NULs, and strcmp would silently compare only up to the
    // first one. Never strcmp a wire string.
    ...
    //
    // IMPORTANT: this callback MUST set *errorCode on EVERY `false` return. The
    // DCC path has no default - the stack pre-initialises errorCode to
    // BACnetErrorCode::success (84, NOT 0) and only names an error class from
    // what we set. This differs from the Get* callbacks above, whose catch-all
    // `false` deliberately leaves errorCode alone; do not carry that habit here.
    return true;
}
...
int main(int argc, char** argv) {
    ...
    // The DeviceCommunicationControl callback makes this a B-AS (DM-DCC-B).
    ...
    // Enable the services a B-AS's DS/DM base must execute: ReadProperty
    // (DS-RP-B) and DeviceCommunicationControl (DM-DCC-B). We set them
    // explicitly to make the profile requirements obvious.
    //
    // We deliberately do NOT enable WriteProperty, ReadPropertyMultiple,
    // SubscribeCOV, or any alarm/event service: they are not part of this
    // profile's required set.
    ...
    // the client reads back Error: unknown-property.
