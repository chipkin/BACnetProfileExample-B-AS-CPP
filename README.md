# BACnet B-AS (Authorization Server) - C++ example

A minimal, copy-paste-friendly example showing how to implement the BACnet
**B-AS (BACnet Authorization Server)** device profile's **DS/DM base** in C++
using the
[CAS BACnet Stack](https://store.chipkin.com/services/stacks/bacnet-stack).
It listens on **BACnet/IP (UDP 47808)**, answers **ReadProperty** requests, is
discoverable via **Who-Is / I-Am** and **Who-Has / I-Have**, and responds to
**DeviceCommunicationControl**.

Part of the CAS BACnet Stack **BACnet profile example series** - one repository
per BACnet device profile. This example claims **only** the B-AS profile's
DS/DM base - see [What this example supports](#what-this-example-supports) for
the one BIBB (AA-AS-B) it does not implement, and why.
...
```bash
git clone --recursive https://github.com/chipkin/BACnetProfileExample-B-AS-CPP.git
cd BACnetProfileExample-B-AS-CPP
tools/build-stack-static.sh BACnetProfileExample-B-AS-CPP    # from the series root; builds the static library
cmake -B build -S . -DCAS_BACNET_STACK_LINK=STATIC
cmake --build build --config Release
./build/Release/BACnetExampleBAS.exe   # Windows; Linux/macOS: ./build/BACnetExampleBAS
```

The rest of this document explains *what a B-AS is* and *why the code is shaped
this way*.

## What is a B-AS (BACnet Authorization Server) profile?
...
**B-AS (BACnet Authorization Server)** (Annex L.14) is the device that other
BACnet devices ask to authorize an access request - the server side of the
Access Authorization exchange defined in ASHRAE 135 Clause 17.
...
- **Discovery:** the device must be findable, so it answers **Who-Is** with
  **I-Am** (DM-DDB-B), and **Who-Has** with **I-Have** (DM-DOB-B).
- **Device Management - DeviceCommunicationControl - B side (DM-DCC-B):**
  respond to a **DeviceCommunicationControl** request, which lets a management
  station stop/resume this device's communication (optionally timed, optionally
  password-protected).
- **Access Authorization - B side (AA-AS-B):** execute the Access
  Authorization service exchange (Clause 17) - **NOT implemented in this
  example**. See [What this example supports](#what-this-example-supports)
  below.

## DeviceCommunicationControl (DM-DCC-B)

`DeviceCommunicationControl` lets a management station tell a device to stop
(or resume) communicating - useful to quiet a device during commissioning. The
CAS BACnet Stack runs the actual enable/disable state machine (and the
optional re-enable timer); this example's `DeviceCommunicationControl`
callback in `main.cpp` validates the optional password and logs what was
asked. At Protocol_Revision 24 the plain "disable" value is deprecated - the
stack itself rejects it and only `enable` and `disable-initiation` take
effect - see the callback's comments in `main.cpp` for the full detail. This
example's password is empty (`DCC_PASSWORD = ""`), so it accepts any request;
set it to require one.
...
"Rainbow"). These are the same base-series sensor objects every example in the
series carries; a real Authorization Server would also carry the Access User /
Access Rights / Access Credential objects Clause 17 defines, but those only
matter once AA-AS-B itself can be executed (see below) - adding them without a
working service exchange behind them would not demonstrate anything.

## What this example supports

The example implements the capabilities below - and, deliberately, not
AA-AS-B. This is **not** the full B-AS profile; it is the DS/DM base plus an
...
| AA-AS-B | Access Authorization - B | ❌ **waiting on stack export** - see [TODO.md](TODO.md) |

**AA-AS-B: waiting on stack export `BACnetStack_AddAuthorizationPolicy`** (and
its siblings `SetAuthorizationPolicyEntry`, `SetAuthorizationPolicyEnabled`,
`AddAuthorizationCache`, `InsertAuthorizationToken`). At the series pin
(`abd4cee1c7f28ca8e1af4720849c4081082bbe82`, 6.0.21) those exports exist only
in `CASBACnetStackTestToolDLL.h`, gated behind `BACNET_STACK_TESTTOOL` - the
stack header itself says they are "a distinct feature that lives on the
test-tool interface," not the customer-facing `CASBACnetStackDLL.h` this
series builds against (see the series' "Customer-facing interface only" rule).
...
executed through the customer DLL surface at this pin. Full evidence,
including the exact header comments, is in [TODO.md](TODO.md). Stack issue:
...
| `MODEL_NAME` | `CAS BACnet Stack Example - B-AS` | Your model designation. This is what a building operator reads to identify your device. |
...
| Device instance | `389023` (`--deviceID` overrides) | Must be unique on the internetwork. BACnet requires this to be configurable; keep it so. |
| `DCC_PASSWORD` | `""` (no password) | A real deployment should set a `DeviceCommunicationControl` password. |
...
```bash
tools/build-stack-static.sh BACnetProfileExample-B-AS-CPP   # from the series root; builds
...
```bash
# Linux / macOS
./build/BACnetExampleBAS

# Windows
.\build\Release\BACnetExampleBAS.exe
```

Expected output:

```
BACnet Authorization Server (B-AS) Example - C++ v1.0.0
...
| `--deviceID <n>` | `389023` | The device's BACnet instance number (BACnet requires this to be configurable). |
...
1. **Discover** - send a **Who-Is**. The device replies with **I-Am** from
   instance **389023** (vendor **389**). It also broadcasts an I-Am at start-up.
...
3. **Browse the object model** - the device shows five objects: the Device
   (`Rainbow`), the three sensors, and the Network Port (`Vermilion`).
...
|---|---|---|---|---|---|---|---|
| Windows x64 (windows-2022) | `BACnetExampleBAS.exe` | 3,247,104 bytes (~3.1 MiB) | `a07a82293644ad5f` | 55 ms | `abd4cee1` | STATIC | Visual Studio 17 2022 |
| Linux x64 (ubuntu-latest) | `BACnetExampleBAS` | 39,536 bytes (~39 KiB) | `8edf1a151c9efcd8` | 109 ms | `abd4cee1` | STATIC | `/usr/bin/c++` |

From release [v1.0.0](https://github.com/chipkin/BACnetProfileExample-B-AS-CPP/releases/tag/v1.0.0) (`metrics-windows.json` / `metrics-linux.json`).

## References

- **ANSI/ASHRAE Standard 135** (BACnet) - the protocol standard. Object model
  (Clause 12), services (Clause 15), Access control authorization (Clause 17),
...
- **The AA-AS-B gap** - [`TODO.md`](TODO.md).
...
See also [CHANGELOG.md](CHANGELOG.md), [AGENTS.md](AGENTS.md), and [TODO.md](TODO.md).
